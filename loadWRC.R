...
ld <- function(url,quiet=T){
  ...
  xmlfile <- xmlParse(RCurl::getURLContent(url = url))
...
for(i in 1:length(sites)){
  ...
  for(j in 1:length(Measurements)){
    ...
    url=gsub(pattern = ' ',replacement = '%20',x = url)
    if(grepl(pattern = ' ',x = url)){browser()}
    xmlfile <- ld(url)
...
# save(Data,file = paste0("H:/ericg/16666LAWA/2018/WaterQuality/1.Imported/",format(Sys.Date(),'%Y-%m-%d'),"/wrcSWQ.Rda"))
...
saveXML(con$value(), file = paste0("H:/ericg/16666LAWA/2018/WaterQuality/1.Imported/",format(Sys.Date(),'%Y-%m-%d'),"/wrcSWQ.xml"))
