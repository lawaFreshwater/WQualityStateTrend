...
message(paste("TDC: Loading data from TDC Hilltop Server",Process))
    ...
    fname <- "H:/ericg/16666LAWA/2018/WaterQuality/R/lawa_state/2018_csv_config_files/tdcSWQ_config.csv"
    ...
    sites = unique(siteTable$CouncilSiteID[siteTable$Agency=='tdc'])
    ...
    nodata <-  function(url){
      xmldata <- ld(url)
      b <- as.character(sapply(getNodeSet(doc=xmldata, path="//E"), xmlValue))
      if(length(b)>0){
        return(TRUE)
      } else {
        return(FALSE)
    ...
    for(i in 1:length(sites)){
      ...
      for(j in 1:length(Measurements)){
        ...
        url <- paste("http://envdata.tasman.govt.nz/WaterQuality.hts?service=Hilltop",
                     "&request=GetData",
                     "&Site=",sites[i],
