...
## To pull the data from Southland's hilltop server, I have a config csv that contains the 
## site and measurement names

fname <- "H:/ericg/16666LAWA/2018/WaterQuality/R/lawa_state/2018_csv_config_files/nccSWQ_config.csv"
...
sites = unique(siteTable$CouncilSiteID[siteTable$Agency=='ncc'])
...
for(i in 1:length(sites)){
  ...
  for(j in 1:length(Measurements)){
    ...
    url <- paste("http://envdata.nelson.govt.nz/data.hts?service=Hilltop",
                 "&request=GetData",
                 "&Site=",sites[i],
