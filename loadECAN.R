...
Process<-TRUE
message(paste("ECAN: Loading data from ECAN Hilltop Server",Process))
# importDestination <- paste("H:/ericg/16666LAWA/2018/WaterQuality/1.Imported/",format(Sys.Date(),"%Y-%m-%d"),"/",sep="")
if(Process){
  if(exists("importDestination")&!file.exists(paste(importDestination,file="ecanSWQ.csv",sep=""))){
    write.csv(c(0),file=paste(importDestination,file="ecanSWQ.csv",sep=""))
    ...
    fname <- "H:/ericg/16666LAWA/2018/WaterQuality/R/lawa_state/2018_csv_config_files/ecanSWQ_config.csv"
    ...
    sites = unique(siteTable$CouncilSiteID[siteTable$Agency%in%c('environment canterbury','christchurch')])
    ...
    requestData <- function(url,quiet=T){
      if(!quiet){
        cat(url,"\n")
      }
    ...
    for(i in 1:length(sites)){
      ...
      for(j in 1:length(Measurements)){
        ...
        url <- paste("http://wateruse.ecan.govt.nz/wqlawa.hts?service=Hilltop", #data.ecan...
                     "&request=GetData",
                     "&Site=",sites[i],
        ...
        #------------------------------------------
        # Request site and measurement data to be returned as an
        # xml object
    ...
    if(exists("importDestination")){
      saveXML(con$value(), paste(importDestination,file="ecanSWQ.xml",sep=""))
    } else {
      saveXML(con$value(), file="ecanSWQ.xml")
    }
