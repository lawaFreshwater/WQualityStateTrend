...
if(Process){
  if(exists("importDestination")&!file.exists(paste(importDestination,file="wcrcSWQ.csv",sep=""))){
    write.csv(c(0),file=paste(importDestination,file="wcrcSWQ.csv",sep=""))
    ...
    write.csv(Data,file = paste(importDestination,file="wcrcSWQ.csv",sep=""),row.names = F)
